...
namespace ConversionTool.Services.Converter
{
    public class TemperatureConverter : IConverter
    {
        ...
        public TemperatureConverter(IConverterAPIFactory converterAPIFactory)
        {
            _converterAPIFactory = converterAPIFactory ?? throw new ArgumentNullException(nameof(converterAPIFactory));
            _converterAPI = _converterAPIFactory.getConverterAPI(SupportedTypes.ConverterTypes.Temperature);
        }
